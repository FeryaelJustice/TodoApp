...
class DeleteTaskUseCase @Inject constructor(private val tasksTodoRepository: TasksTodoRepository) {
    suspend operator fun invoke(task: Task) = tasksTodoRepository.deleteTask(task)
}